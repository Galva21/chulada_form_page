...
import 'package:chulada_form_page/questions/male/qf14.dart';
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class QuestionFemale133 extends StatefulWidget {
  const QuestionFemale133({super.key});

  @override
  State<QuestionFemale133> createState() => _QuestionFemale133State();
}

class _QuestionFemale133State extends State<QuestionFemale133> {
  ...
  ElevatedButton BotonWidget1() {
    return ElevatedButton(
      onHover: (value) {
        if (value) {
          setState(() {
            opcionSeleccionado1 = Colors.amber.shade500;
      ...
      onPressed: () {
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const QuestionFemale144()),
        );
      },
  ...
  ElevatedButton BotonWidget2() {
    return ElevatedButton(
      onHover: (value) {
        if (value) {
          setState(() {
            opcionSeleccionado2 = Colors.amber.shade500;
            colorTexto = Colors.white;
          });
        } else {
          setState(() {
  ...
  ElevatedButton BotonWidget3() {
    return ElevatedButton(
      onHover: (value) {
        if (value) {
          setState(() {
            opcionSeleccionado3 = Colors.amber.shade500;
            colorTexto = Colors.white;
          });
        } else {
          setState(() {
  ...
  ElevatedButton BotonWidget4() {
    return ElevatedButton(
      onHover: (value) {
        if (value) {
          setState(() {
            opcionSeleccionado4 = Colors.amber.shade500;
            colorTexto = Colors.white;
          });
        } else {
          setState(() {
  ...
  ElevatedButton BotonWidget5() {
    return ElevatedButton(
      onHover: (value) {
        if (value) {
          setState(() {
            opcionSeleccionado5 = Colors.amber.shade500;
            colorTexto = Colors.white;
          });
        } else {
          setState(() {
